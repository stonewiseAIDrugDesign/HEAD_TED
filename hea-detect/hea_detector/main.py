...
class HEAD:
    ...
    def load_ani2x_model(self):
        ...
        model = ANI2x(periodic_table_index=False, model_index=None).to(self.device)
    ...
    def load_molecules_from_file(
        ...
        if file_extension == "pdb":
            # Load PDB
            if add_Hs:
                mol = MolFromPDBFile(file_path, removeHs=True, sanitize=sanitize)
                mol = Chem.AddHs(mol, addCoords=True)
            else:
                mol = MolFromPDBFile(file_path, removeHs=remove_Hs, sanitize=sanitize)
            if mol is None:
            ...
            # Load SDF
            if add_Hs:
                suppl = SDMolSupplier(file_path, removeHs=True, sanitize=sanitize)
            else:
                suppl = SDMolSupplier(file_path, removeHs=remove_Hs, sanitize=sanitize)
            molecules = []
            for mol in suppl:
                if mol is not None:
                    if add_Hs:
