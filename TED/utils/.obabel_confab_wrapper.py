...
def obabel_wrapper(input_sdf, xcutoff, ecutoff, conf_num, workdir='.'):
...
def obabel_mp(input_sdf_list, xcutoff, ecutoff, conf_num, workdir, num_proc):
    ...
    with mp.Pool(num_proc) as workers:
        res = list(workers.imap(obj, input_sdf_list))
    return res
