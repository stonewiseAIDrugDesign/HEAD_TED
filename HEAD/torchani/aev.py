...
has_cuaev = False #'torchani.cuaev' in importlib_metadata.metadata(__package__).get_all('Provides')
...
class AEVComputer(torch.nn.Module):
    ...
    def __init__(self, file_name,Rcr, Rca, EtaR, ShfR, EtaA, Zeta, ShfA, ShfZ, num_species, use_cuda_extension=False):
        super().__init__()
    ...
    def forward(self, input_: Tuple[Tensor, Tensor],
        ...
        # print(aev.shape)
        # print(aev)
        #np.save(f'output/aev_{File_name.FIlE_NAME}.npy',aev.cpu().detach().numpy())
        Temp.AEV = aev.cpu().detach().numpy()
        return SpeciesAEV(species, aev)
