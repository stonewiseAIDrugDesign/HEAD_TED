...
class ANIModel(torch.nn.ModuleDict):
    ...
    def forward(self, species_aev: Tuple[Tensor, Tensor],  # type: ignore
        ...
        # print('atomic_energie:',torch.sum(atomic_energies, dim=1))
        # shape of atomic energies is (C, A) 
        return SpeciesEnergies(species, torch.sum(atomic_energies, dim=1)),atomic_energies
    ...
    def _atomic_energies(self, species_aev: Tuple[Tensor, Tensor]) -> Tensor:
        ...
        species, aev = species_aev
        assert species.shape == aev.shape[:-1]
        species_ = species.flatten() # tensor([1, 0, 0, 0, 0])
        aev = aev.flatten(0, 1) # 1*n*384 变成了n*384
        output = aev.new_zeros(species_.shape)
        # self.values():模型
        for i, m in enumerate(self.values()):
            # i为0时,mask:[False, True, True,  True,  True],midx:[1, 2, 3, 4]
            # i为1时,mask:[True, False, False, False, False] , midx:[0]
            mask = (species_ == i) 
        ...
        # print('output:',output)
        # np.save(f'energies/atomic_energies_{timestamp}.npy',output.detach().numpy())
        return output
...
class Ensemble(torch.nn.ModuleList):
    ...
    def __init__(self, modules,file_name):
    ...
    def forward(self, species_input: Tuple[Tensor, Tensor],  # type: ignore
        ...
        start = time.time()
        for x in self:
            speciesEnergies, atomic_energies=x(species_input)
            sum_+=speciesEnergies[1]
            atomic_energies_list.append(atomic_energies)
        species, _ = species_input
        # print('model time:',time.time()-start)
        stacked_atomic_energies = torch.stack(atomic_energies_list, dim=0)
        final_atomic_energies = torch.sum(stacked_atomic_energies , dim=0)/self.size
        
        Temp.ATOMIC_ENERGIES = final_atomic_energies.cpu().detach().numpy()
        #print('FILE_NAME:', File_name.FIlE_NAME)

        #np.save(f'output/energies_{File_name.FIlE_NAME}.npy',final_atomic_energies.cpu().detach().numpy())
        # print('final_atomic_energies:',final_atomic_energies)
        # print('###########',sum_ / self.size)
