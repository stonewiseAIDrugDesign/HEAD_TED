...
from . import min_energy_fro_torsion
from . import scan_xtb_gen_feature
from . import flexible_scan_with_mmff94
from . import model_inference_with_rdkit_mmff94
...
def run_the_service_api(data_path, model_path, out_path):
    input_dict_list = read_data_to_input_list(data_path)
    unique_id = str(uuid.uuid4()).replace('-', '_')
    print(f'unique id:{unique_id}')
    task_id = f'default_{unique_id}'
    functionExtraParam = {}
    functionExtraParam['task_name'] = task_id
    functionExtraParam['model_path'] = model_path
    output_cols = ['mol_id', 'unique_key', 'energy']
    output_dict_list = udf_run(input_dict_list, output_cols, functionExtraParam)
    output_data_to_csv(out_path, output_dict_list)
